...
-------------------------------------------'''
        ...
        print u"对不起，程序第一次运行，尚无历史记录可打印。你可以选择2开始输入新内容。"
        main()

def writenew():
    # 输入新的内容
    txt = open("tempfile.txt", 'a')
    newline = raw_input(u"今日记录，请输入： ")
...
2 不想，只是随便写写。    '''
        ...
        if choose =="1" :
            filename = raw_input(u"存储今日记录，请输入文件名：")
